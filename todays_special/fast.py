...
app = FastAPI(title="Mini Calc API", version="1.0")
...
@app.get("/add", response_model=Result, summary="Add two numbers via query params")
def add_get(a: int, b: int):
    return {"result": a + b}

# NEW: POST endpoint using JSON body
@app.post("/add", response_model=Result, summary="Add two numbers via JSON")
def add_post(payload: AddRequest):
    return {"result": payload.a + payload.b}

# Bonus: Batch calculator to show validation + enums
@app.post("/calc", response_model=Result, summary="Batch add/multiply numbers via JSON")
